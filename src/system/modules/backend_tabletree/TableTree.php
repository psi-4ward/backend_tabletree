...
//print_r($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']);
    ...
    public function setTableHeader()
    {
        ...
        // get icons
        $tableColumn = $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['tableColumn'];
        if (is_array($tableColumn))
        {
            $this->import($tableColumn[0]);
            $tableColumn = $this->$tableColumn[0]->$tableColumn[1]($this->dataContainer);
        }
        list($sourceTable, $sourceColumn) = explode('.', $tableColumn);
